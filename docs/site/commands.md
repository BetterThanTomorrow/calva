...
| :------ | :---- | :-------- | :---- |
| `calva.loadFile` | Load/Evaluate Current File and its Requires/Dependencies | `{ path?: string \| string[] \| Uri, silent?: boolean, sessionKey?: string, who?: string }` | Without arguments, or without `path`, loads the active editor's file. `path` can be absolute, workspace-relative, path segments, or a `Uri`. `silent` suppresses UI error dialogs and rejects the promise instead. `sessionKey` targets a specific REPL session (defaults to the session routed for the current editor context). `who` identifies the caller for multi-party session tracking (defaults to `"ui"`). Returns the evaluation result. |
