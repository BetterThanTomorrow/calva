...
const instrumentedSymbolDecorationType =
...
async function update(
    ...
    if (/(\.clj)$/.test(editor.document.fileName)) {
        if (cljSession && util.getConnectedState() && lspClient) {
            const instrumentedDefLists = (
                await cljSession.listDebugInstrumentedDefs()
            ).list;

            instrumentedSymbolReferenceLocations =
                await instrumentedDefLists.reduce(
                    async (
                        iSymbolRefLocations: Promise<InstrumentedSymbolReferenceLocations>,
                        [namespace, ...instrumentedDefs]: string[]
                    ) => {
                        const namespacePath = (
                            await cljSession.nsPath(namespace)
                        ).path;
                        const docUri = vscode.Uri.parse(namespacePath, true);
                        const decodedDocUri = decodeURIComponent(
                            docUri.toString()
                        );
                        const docSymbols = (
                            await lsp.getDocumentSymbols(
                                lspClient,
                                decodedDocUri
                            )
                        )[0].children;
                        ...
                        const instrumentedDocSymbolsReferenceRanges =
                            await Promise.all(
                        ...
                        const currentNsSymbolsReferenceLocations =
                            instrumentedDocSymbols.reduce(
                                (currentLocations, symbol, i) => {
                                    return {
                                        ...currentLocations,
                                        [symbol.name]:
                                            instrumentedDocSymbolsReferenceRanges[
                                                i
                                            ],
                                    };
                                },
...
function render(editor: vscode.TextEditor): void {
    ...
    const nsSymbolReferenceLocations = locations.filter(
        (loc) =>
            ...
            decodeURIComponent(editor.document.uri.toString())
    );
    ...
    editor.setDecorations(
        instrumentedSymbolDecorationType,
        editorDecorationRanges
    );
}
