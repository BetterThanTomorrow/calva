...
class CalvaDebugSession extends LoggingDebugSession {
    ...
    protected attachRequest(
        ...
        state
            .analytics()
            .logEvent(
                DEBUG_ANALYTICS.CATEGORY,
                DEBUG_ANALYTICS.EVENT_ACTIONS.ATTACH
            )
            .send();
    }
    ...
    protected continueRequest(
        ...
        state
            ...
            .logEvent(
                DEBUG_ANALYTICS.CATEGORY,
                DEBUG_ANALYTICS.EVENT_ACTIONS.CONTINUE
            )
            .send();
    }
    ...
    protected nextRequest(
        ...
        state
            ...
            .logEvent(
                DEBUG_ANALYTICS.CATEGORY,
                DEBUG_ANALYTICS.EVENT_ACTIONS.STEP_OVER
            )
            .send();
    }
    ...
    protected stepInRequest(
        ...
        state
            ...
            .logEvent(
                DEBUG_ANALYTICS.CATEGORY,
                DEBUG_ANALYTICS.EVENT_ACTIONS.STEP_IN
            )
            .send();
    }
    ...
    protected stepOutRequest(
        ...
        state
            ...
            .logEvent(
                DEBUG_ANALYTICS.CATEGORY,
                DEBUG_ANALYTICS.EVENT_ACTIONS.STEP_OUT
            )
            .send();
    }
    ...
    private async _showDebugAnnotation(
        ...
        const editor =
            visibleEditor || (await vscode.window.showTextDocument(document));
    ...
    protected async stackTraceRequest(
        ...
        const source = new Source(
            basename(debugResponse.file),
            debugResponse.file
        );
        const name = tokenCursor.getFunctionName();
        const stackFrames = [
            new StackFrame(0, name, source, line + 1, column + 1),
        ];
...
class CalvaDebugAdapterDescriptorFactory
...
function onNreplMessage(data: any): void {
    if (vscode.debug.activeDebugSession && (data['value'] || data['err'])) {
        ...
        data['status'] &&
        data['status'].indexOf(NEED_DEBUG_INPUT_STATUS) !== -1
    ) {
