...
function symbolChar(): fc.Arbitrary<string> {
    ...
    return fc
        .unicode()
        .filter((c) => !(nonSymbolChars.includes(c) || c.match(/\s/)));
}
...
function symbol(): fc.Arbitrary<string> {
    return fc
        .tuple(symbolStart(), fc.stringOf(symbolChar(), 1, 5))
        .map(([c, s]) => `${c}${s}`);
}
...
function underscoreSymbol(): fc.Arbitrary<string> {
    return fc
        .tuple(
            fc.constant('_'),
            symbolStartIncludingDigit(),
            fc.stringOf(symbolChar(), 1, 5)
        )
        .map(([c, s]) => `${c}${s}`);
}
...
function quotedUnicode(): fc.Arbitrary<string> {
    return fc
        .tuple(fc.constantFrom('\\'), fc.unicode())
        .map(([c, s]) => `${c}${s}`);
}
