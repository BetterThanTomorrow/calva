...
class ProjectTreeDataProvider implements vscode.TreeDataProvider<ProjectTreeNode>, vscode.TextDocumentContentProvider {
    ...
    private _elementTypeToIcon(type: ProjectTreeNodeType): vscode.ThemeIcon {
        switch (type) {
            case ProjectTreeNodeType.project: return vscode.ThemeIcon.Folder
            case ProjectTreeNodeType.sourcePath: return new vscode.ThemeIcon('file-directory')
            case ProjectTreeNodeType.library: return new vscode.ThemeIcon('library')
            case ProjectTreeNodeType.jar: return vscode.ThemeIcon.File
            case ProjectTreeNodeType.class: return new vscode.ThemeIcon('symbol-class')
            case ProjectTreeNodeType.ns: return vscode.ThemeIcon.File
            case ProjectTreeNodeType.interface: return new vscode.ThemeIcon('symbol-interface')
            case ProjectTreeNodeType.function: return new vscode.ThemeIcon('symbol-function')
            case ProjectTreeNodeType.variable: return new vscode.ThemeIcon('symbol-variable')
            default: vscode.ThemeIcon.File;
        }
    }
    ...
    getTreeItem(element: ProjectTreeNode): vscode.TreeItem {
        ...
        return {
            ...
            collapsibleState: isFinal ? vscode.TreeItemCollapsibleState.None : vscode.TreeItemCollapsibleState.Collapsed,
            command: element.uri ? {
    ...
    getChildren(element?: ProjectTreeNode): vscode.ProviderResult<ProjectTreeNode[]> {
        return this.client.sendRequest<ProjectTreeNodeBranch>('clojure/workspace/projectTree/nodes', element)
...
export class ProjectTreeExplorer {
    ...
    private async _navigateToNode(element: ProjectTreeNode) {
        vscode.commands.executeCommand('vscode.open', vscode.Uri.parse(element.uri))
            .then(() => {
                if ('range' in element && element.range) {
                    if (vscode.window.activeTextEditor) {
                        const newPos = new vscode.Position(element.range.start.line, element.range.start.character);
