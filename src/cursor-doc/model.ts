...
export class LineInputModel implements EditableModel {
    ...
    constructor(
        readonly lineEndingLength: number = 1,
    ...
    flushChanges() {
        ...
        while (this.dirtyLines.length) {
            ...
            do {
                ...
                this.lines[++nextIdx] &&
                !equal(this.lines[nextIdx].startState, prevState)
            );
        }
    }
    ...
    getWordSelection(offset: number): [number, number] {
        ...
        if (text && text.length > 1 && column < text.length && column >= 0) {
            ...
            return [
                offset - (column - startIdx),
                offset + (stopIdx - column) + 1,
            ];
        }
        return [offset, offset];
    }
    ...
    private changeRange(
        ...
        if (currentLength > replaceLines.length) {
            // shrink the lines
            this.deleteLines(
                startLine + replaceLines.length,
                currentLength - replaceLines.length
            );
