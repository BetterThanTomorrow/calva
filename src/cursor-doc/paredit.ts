...
export function moveToRangeLeft(
    doc: EditableDocument,
    range: [number, number]
) {
...
export function moveToRangeRight(
    doc: EditableDocument,
    range: [number, number]
) {
...
export function selectBackwardUpSexp(doc: EditableDocument) {
    const rangeFn =
        doc.selection.active <= doc.selection.anchor
            ? (doc: EditableDocument) =>
                  rangeToBackwardUpList(doc, doc.selection.active, false)
            : (doc: EditableDocument) =>
                  rangeToBackwardUpList(doc, doc.selection.active, false);
    selectRangeBackward(doc, rangeFn(doc));
}
...
export function forwardHybridSexpRange(
    ...
    if (
        currentLineNewlineOffset > cursor.offsetEnd &&
        currentFormEndToken.type != 'close'
    ) {
        hasNewline = true;
...
export function splitSexp(
    ...
    if (
        !cursor.withinString() &&
        !(cursor.isWhiteSpace() || cursor.previousIsWhiteSpace())
    ) {
        cursor.forwardWhitespace();
    }
...
export function joinSexp(
    ...
    if (['close', 'str-end', 'str'].includes(prevToken.type)) {
        ...
        if (
            validPair(nextToken.raw[0], prevToken.raw[prevToken.raw.length - 1])
        ) {
            return doc.model.edit(
                [
                    new ModelEdit('changeRange', [
            ...
            return doc.model.edit(
                [
                    new ModelEdit('changeRange', [
                        ...
                        end + close.raw.length,
                        '',
                    ]),
                    new ModelEdit('changeRange', [
                        beginning - open.raw.length,
                        beginning,
                        '',
                    ]),
                ],
        ...
        if (cursor.forwardSexp(true, true)) {
            ...
            const changeArgs =
                replacedText.indexOf('\n') >= 0
                    ? [
                          currentCloseOffset,
                          currentCloseOffset + close.length,
                          '',
                      ]
            ...
            const formatDepth = extraOpts['formatDepth']
                ? extraOpts['formatDepth']
                : 1;
    ...
    if (doc.selectionStack.length > 0) {
        ...
        if (
            !(
                ...
                doc.selectionRight == prev.active
            )
        ) {
                        ...
                        if (
                            ...
                            void doc.model.edit(
                                [
                                    ...
                                    new ModelEdit('changeRange', [
                                        cursorEnd.offsetStart,
                                        cursorEnd.offsetEnd,
                                        '',
                                    ]),
                                    new ModelEdit('changeRange', [
                                        cursorStart.offsetStart,
                                        end,
                                        '',
                                    ]),
                                    new ModelEdit('changeRange', [
    ...
    if (usePairs) {
        ...
        if (ranges.length > 1) {
            const indexOfCurrentSingle = ranges.findIndex(
                (r) =>
                    r[0] === currentSingleRange[0] &&
                    r[1] === currentSingleRange[1]
            );
...
export function dragSexprBackward(
    ...
    const backRange = currentSexpsRange(
        doc,
        backCursor,
        backCursor.offsetStart,
        usePairs
    );
...
export function dragSexprBackwardUp(
    ...
    if (cursor.backwardList() && cursor.backwardUpList()) {
        ...
        if (wsInfo.hasLeftWs) {
            ...
            const havePrecedingLineComment =
                lineCommentCursor.getPrevToken().type === 'comment';
            const wsLeftStart =
                wsInfo.leftWsRange[0] + (havePrecedingLineComment ? 1 : 0);
            deleteEdit = new ModelEdit('deleteRange', [
                wsLeftStart,
                currentRange[1] - wsLeftStart,
            ]);
        } else {
            dragText =
                doc.model.getText(...currentRange) +
...
export function dragSexprBackwardDown(
    ...
    while (cursor.backwardSexp()) {
        ...
        if (token.type === 'close') {
            ...
            const listEnd = cursor.offsetStart;
            cursor.backwardWhitespace();
            const siblingWsInfo = collectWhitespaceInfo(
                doc,
                cursor.offsetStart
            );
...
export function addRichComment(
    ...
    const insideNextTopLevelFormPos = rangeToForwardDownList(
        doc,
        insertStart
    )[1];
    if (!contents && insideNextTopLevelFormPos !== insertStart) {
        const checkIfRichCommentExistsCursor = doc.getTokenCursor(
            insideNextTopLevelFormPos
        );
