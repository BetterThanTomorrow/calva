...
function normalizeNewLinesAndJoin(
    strings: string[],
    asLineComment = false
): string {
    return strings
...
function _currentSelectionElseCurrentForm(
    editor: vscode.TextEditor
    ...
    if (util.getConnectedState()) {
        ...
        if (session) {
            try {
                ...
                chan.appendLine(
                    `REPL utilities are now available in namespace ${ns}.`
                );
            } catch (e) {
                chan.appendLine(
                    `REPL utilities could not be acquired for namespace ${ns}: ${e}`
                );
            }
        }
    } else {
...
async function togglePrettyPrint() {
    ...
    if (
        ...
        !(pprintOptions.printEngine || pprintOptions.printFn)
    ) {
...
function instrumentTopLevelForm() {
    ...
    state
        ...
        .logEvent(
            ...
            DEBUG_ANALYTICS.EVENT_ACTIONS.INSTRUMENT_FORM
        )
        .send();
}
