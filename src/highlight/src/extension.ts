...
function reloadConfig() {
    ...
    if (
        !isEqual(
            misplacedBracketStyle,
            configuration.get('misplacedBracketStyle')
        )
    ) {
    ...
    if (
        !isEqual(matchedBracketStyle, configuration.get('matchedBracketStyle'))
    ) {
    ...
    if (
        enableBracketColors !==
        configuration.get<boolean>('enableBracketColors')
    ) {
    ...
    if (
        highlightActiveIndent !==
        configuration.get<boolean>('highlightActiveIndent')
    ) {
        highlightActiveIndent = configuration.get<boolean>(
            'highlightActiveIndent'
        );
        dirty = true;
    }
    ...
    if (
        useRainbowIndentGuides !==
        configuration.get<boolean>('rainbowIndentGuides')
    ) {
        useRainbowIndentGuides = configuration.get<boolean>(
            'rainbowIndentGuides'
        );
        dirty = true;
    }
...
function updateRainbowBrackets() {
    ...
    activeEditor.visibleRanges.forEach((range) => {
        ...
        const cursor: LispTokenCursor =
            mirrorDoc.getTokenCursor(startPaintingFrom);
        do {
            cursor.forwardWhitespace();
            {
                ...
                if (token.type === 'str-inside' || token.raw.includes('"')) {
                    ...
                    const ignore_start = activeEditor.document.positionAt(
                        ignoreCursor.offsetStart
                    );
            ...
            if (token.type === 'open') {
                ...
                if (
                    pair_idx === undefined ||
                    pair_idx < 0 ||
                    !validPair(stack[pair_idx].char, char)
                ) {
                    ...
                    if (in_comment_form && pair.opens_comment_form) {
                        comment_forms.push(
                            new Range(pair.start, pos.translate(0, charLength))
                        );
                    ...
                    if (guideColorsEnabled || activeGuideEnabled) {
                        ...
                        const openSelection = matchBefore(
                        ...
                        if (guideLength > 0) {
                            placedGuidesColor.set(
                                position_str(openSelectionPos),
                                colorIndex(stack_depth)
                            );
                        }
                    }
                    continue;
...
function decorateGuide(
    ...
    for (
        let lineDelta = 1;
        lineDelta <= endPos.line - startPos.line;
        lineDelta++
    ) {
        const guidePos = startPos.translate(lineDelta, 0);
        if (
            doc.lineAt(guidePos).text.match(/^ */)[0].length >=
            startPos.character
        ) {
...
export function activate(context: vscode.ExtensionContext) {
    ...
    vscode.window.onDidChangeTextEditorSelection(
        (event) => {
            if (
                event.textEditor === tryToGetActiveTextEditor() &&
