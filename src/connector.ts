...
async function readJarContent(uri: string) {
    try {
        ...
        const conf = await zipData
            .file('calva.exports/config.edn')
            ?.async('string');
        return [uri, conf];
    } catch (error) {
        return [uri, null];
    ...
    try {
        ...
        nClient = await NReplClient.create({
            ...
            onError: (e) => {
                ...
                if (scheme === 'vsls') {
                    outputWindow.append(
                        '; nREPL connection failed; did the host share the nREPL port?'
                    );
                }
            },
        });
            ...
            if (
                ...
                state
                    ...
                    .logEvent(
                        ...
                        'ConnectCljsRepl',
                        isBuiltinType
                            ? (connectSequence.cljsType as string)
                            : 'Custom'
                    )
                    .send();
            }
...
function createCLJSReplType(
        ...
        startAppNowProcessor: processOutputFn = (x) => {
            ...
            if (appURL && haveShownStartMessage && !haveShownAppURL) {
                if (cljsType.shouldOpenUrl) {
                    outputWindow.append(
                        `; Opening ClojureScript app in the browser at: ${appURL} ...`
                    );
                    open(appURL).catch((reason) => {
                        outputWindow.append(
                            '; Error opening ClojureScript app in the browser: ' +
                                reason
                        );
                    });
                } else {
                    outputWindow.append(
                        ';   Open the app on this URL: ' + appURL
                    );
                }
            ...
            if (
                ...
                (haveShownAppURL ||
                    (haveShownStartMessage && !cljsType.openUrlRegExp))
            ) {
                outputWindow.append(
    ...
    const replType: ReplType = {
        ...
        connect: async (session, name, checkFn) => {
            ...
            if (
                menuSelections &&
                menuSelections.cljsDefaultBuild &&
                useDefaultBuild
            ) {
                ...
                if (
                    typeof initCode === 'object' ||
                    initCode.includes('%BUILD%')
                ) {
            ...
            if (!hasStarted) {
                if (startCode.includes('%BUILDS')) {
                    ...
                    if (menuSelections && menuSelections.cljsLaunchBuilds) {
                        ...
                        const allBuilds = await figwheelOrShadowBuilds(
                            cljsTypeName
                        );
                        builds =
                            allBuilds.length <= 1
                                ...
                                : await util.quickPickMulti({
                                      values: allBuilds,
                                      placeHolder:
                                          'Please select which builds to start',
                                      saveAs: `${state
                                          .getProjectRootUri()
                                          .toString()}/${cljsTypeName.replace(
                                          ' ',
                    ...
                    outputWindow.append(
                        '; Starting cljs repl for: ' + projectTypeName + '...'
                    );
                    return evalConnectCode(
                        session,
                        startCode,
...
async function makeCljsSessionClone(
    ...
    if (newCljsSession) {
        outputWindow.append(
            '; Connecting cljs repl: ' + projectTypeName + '...'
        );
        outputWindow.append(
        ...
        if (repl.start != undefined) {
            if (await repl.start(newCljsSession, repl.name, repl.started)) {
                state
                    .analytics()
                    .logEvent('REPL', 'StartedCLJS', repl.name)
                    .send();
                ...
                state
                    .analytics()
                    .logEvent('REPL', 'FailedStartingCLJS', repl.name)
                    .send();
        ...
        if (await repl.connect(newCljsSession, repl.name, repl.connected)) {
            state
                .analytics()
                .logEvent('REPL', 'ConnectedCLJS', repl.name)
                .send();
            setStateValue('cljs', (cljsSession = newCljsSession));
...
export default {
    ...
    toggleCLJCSession: () => {
        ...
        if (getStateValue('connected')) {
            if (
                replSession.getSession('cljc') == replSession.getSession('cljs')
            ) {
                newSession = replSession.getSession('clj');
            } else if (
                replSession.getSession('cljc') == replSession.getSession('clj')
            ) {
    ...
    switchCljsBuild: async () => {
        ...
        const cljsTypeName: string = state.extensionContext.workspaceState.get(
            ...
            cljTypeName: string = state.extensionContext.workspaceState.get(
                'selectedCljTypeName'
            );
        state
            .analytics()
