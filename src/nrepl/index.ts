...
function resultHandler(resolve: any, reject: any) {
    return (res: any): boolean => {
        if (hasStatus(res, 'unknown-op')) {
            reject(
                "The server does not recognize or cannot perform the '" +
                    res.op +
                    "' operation"
            );
        } else {
...
export class NReplSession {
        ...
        if (vscode.debug.activeDebugSession && this.replType === 'clj') {
            ...
            state
                ...
                .logEvent(
                    ...
                    debug.DEBUG_ANALYTICS.EVENT_ACTIONS
                        .EVALUATE_IN_DEBUG_CONTEXT
                )
                .send();
            return {
                id: debugResponse.id,
    ...
    complete(ns: string, symbol: string, context?: string) {
        return new Promise<any>((resolve, reject) => {
            const id = this.client.nextId,
                extraOpts = getConfig().enableJSCompletions
    ...
    sendDebugInput(
        ...
        return new Promise<any>((resolve, reject) => {
            this.messageHandlers[debugResponseId] = resultHandler(
                resolve,
                reject
            );
    ...
    onMessage(msg: any, pprintOptions: PrettyPrintingOptions): boolean {
        ...
        if (msg) {
            ...
            if (hasStatus(msg, 'done') || hasStatus(msg, 'need-debug-input')) {
                ...
                if (
                    this.exception &&
                    this.msgValue !== debug.DEBUG_QUIT_VALUE
                ) {
                    this.session
                        .stacktrace()
                        .then((stacktrace) => {
                    ...
                    if (
                        ...
                        pprintOptions.printEngine === 'calva'
                    ) {
