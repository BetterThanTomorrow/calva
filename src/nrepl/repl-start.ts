...
const DRAM_BASE_URL =
    'https://raw.githubusercontent.com/BetterThanTomorrow/dram';
...
async function downloadDram(
    ...
    const dirUri = vscode.Uri.joinPath(
        storageUri,
        path.join(...path.dirname(filePath).split(/\//))
    );
    await vscode.workspace.fs.createDirectory(dirUri);
    const storeFileUri = vscode.Uri.joinPath(
        storageUri,
        path.join(...filePath.split(/\//))
    );
    return await utilities
        .downloadFromUrl(downloadUrl, storeFileUri.fsPath)
        .catch((err) => {
...
async function openStoredDoc(
    ...
    const destUri = vscode.Uri.file(
        path.join(tempDirUri.fsPath, dramFile.path)
    );
    try {
...
export async function startStandaloneRepl(
    ...
    const [mainDoc, mainEditor] = await openStoredDoc(
        storageUri,
        tempDirUri,
        config.files[0]
    );
...
export async function startOrConnectRepl() {
    ...
    const START_HELLO_CLJS_BROWSER_OPTION =
        'Fire up the ”ClojureScript Quick Start” Browser REPL';
    const START_HELLO_CLJS_BROWSER_COMMAND =
        'calva.startStandaloneCljsBrowserRepl';
    const START_HELLO_CLJS_NODE_OPTION =
    ...
    const CONNECT_STANDALONE_OPTION =
        'Connect to a running REPL, not in your project';
    ...
    if (
        ...
        if (
            ...
            if (vscode.workspace.workspaceFolders[0].uri.scheme != 'vsls') {
                ...
                commands[CONNECT_STANDALONE_OPTION] =
        ...
        commands[START_HELLO_CLJS_BROWSER_OPTION] =
    ...
    const sortedCommands = utilities.sortByPresetOrder(
