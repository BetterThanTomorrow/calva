...
async function fetchConfig(configName: string): Promise<DramConfig> {
    const configEdn = await utilities.fetchFromUrl(`${DRAM_BASE_URL}/dev/drams/${configName}/dram.edn`);
...
export async function downloadDrams(storageUri: vscode.Uri, configPath: string,  filePaths: string[]) {
...
async function openStoredDoc(storageUri: vscode.Uri, tempDirUri: vscode.Uri, dramFile: DramFile): Promise<[vscode.TextDocument, vscode.TextEditor]> {
...
export async function startStandaloneRepl(context: vscode.ExtensionContext, dramTemplate: DramTemplate, areBundled: boolean) {
    let config = typeof (dramTemplate.config) === 'string' ? await fetchConfig(dramTemplate.config) : dramTemplate.config;
